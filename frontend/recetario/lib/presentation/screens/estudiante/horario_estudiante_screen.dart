...
class HorarioEstudianteScreen extends StatefulWidget {
  const HorarioEstudianteScreen({Key? key}) : super(key: key);

  @override
  State<HorarioEstudianteScreen> createState() => _HorarioEstudianteScreenState();
}

class _HorarioEstudianteScreenState extends State<HorarioEstudianteScreen> {
  ...
  Future<void> _cargarHorario() async {
    ...
    try {
      ...
      final estudianteId = userData['id'];
      final horarios = await HorarioService.getHorarioEstudiante(token, estudianteId);
  ...
  Widget _buildHeaderDias(bool isMobile) {
    ...
    final colHorasWidth = isMobile ? 90.0 : 80.0;
  ...
  Widget _buildGridHorarios(bool isMobile) {
    // ✅ Adaptación responsiva
    final cellWidth = isMobile ? 70.0 : 120.0;
    final cellHeight = isMobile ? 70.0 : 90.0;
    final colHorasWidth = isMobile ? 90.0 : 80.0;
